...
describe('Bank Resource Limits & Scarcity Rule', () => {
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('client:maritime_trade', ({ give, receive }) => {
                ...
                if (state.bank[resourceType] >= amount) {
                    ...
                    socket.emit('server:action_invalid', { reason: 'Bank does not have enough resources' });
                }
            });
    ...
    it('should reject a maritime trade if the bank does not have enough of the requested resource', async () => {
        ...
        const invalid = new Promise<void>((resolve) => {
            clientA.once('server:action_invalid', (data) => {
                ...
                expect(data.reason).toMatch(/bank.*enough/i);
                resolve();
            });
        });
